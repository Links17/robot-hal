...
Make every Robot HAL deployment form preserve one structured error model without introducing gRPC or a second error framework. The change must remain additive within wire major version 1 and must not alter the stable error decisions already used by callers.
...
`robot-hal-protocol` owns both Rust conversion directions. The Rust client must call the protocol conversion rather than maintain a second mapping for `v1::Error`.
