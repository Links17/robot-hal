...
- GREEN: `cargo test -p seeed-hal-core --test core_contract` → 11 passed;
  `cargo test -p seeed-hal-testkit --test serial_conformance` → 9 passed.
...
- Files: `crates/seeed-hal-core/src/{capability,identity,lib}.rs`,
  `crates/seeed-hal-serial/src/lib.rs`, `adapters/serialport/src/lib.rs`,
  `crates/seeed-hal-testkit/src/virtual_serial.rs`, protocol descriptor conversion, and their tests.
...
- GREEN: `cargo test -p seeed-hal-runtime --test serial_runtime` → 16 passed, including 4,096
...
- Files: `crates/seeed-hal-runtime/src/{lease_table,registry,lib}.rs` and
  `crates/seeed-hal-runtime/tests/serial_runtime.rs`.
...
- GREEN: `cargo test -p seeed-hal-protocol --test protocol_contract` → 7 passed;
...
- Files: `proto/seeed/hal/v1/hal.proto`, generated Python binding,
  `crates/seeed-hal-protocol`, broker/client handshake code, Python client, manifest, and black-box
  runner.
...
- GREEN: `cargo test -p seeed-hal-windows-security` → 2 platform-neutral tests passed;
...
- GREEN: `cargo test -p seeed-hal-core --test core_contract
...
- Files: `crates/seeed-hal-core/src/error.rs` and core contract tests.
...
- RED: `StartupToken` exposed ordinary derived `PartialEq/Eq` in addition to a local constant-time
  comparison.
- GREEN: `cargo test -p seeed-hal-broker --lib
...
- Files: `crates/seeed-hal-broker/src/{lib,connection}.rs`.

### 8. Complete v1 protobuf tag locks

- RED: the previous contract locked only the handshake request tag.
- GREEN: `cargo test -p seeed-hal-protocol --test protocol_contract
...
- Files: `crates/seeed-hal-protocol/tests/protocol_contract.rs`.
...
- GREEN: `cargo test -p seeed-hal-broker-app
...
- Files: `apps/seeed-hal-broker/src/main.rs` and its tests.
...
- RED: cleanup guards were created after a fallible permission change, so that failure could strand
  the bound socket.
- GREEN: `cargo test -p seeed-hal-broker-app unix_socket_cleanup_is_armed_immediately_after_bind`
  → 1 passed; `cargo test -p seeed-hal-broker --test broker_contract
...
- Files: `apps/seeed-hal-broker/src/main.rs`,
  `crates/seeed-hal-broker/src/listener/unix.rs`, and broker contract tests.
...
| Virtual manifest | `cargo test -p seeed-hal-broker-app --features virtual-adapter --test manifest` | 1 passed |
| Broker build | `cargo build -p seeed-hal-broker-app --features virtual-adapter` | Passed |
| Local black-box | `uv run --project bindings/python --frozen python tests/conformance/run-broker-conformance.py --broker target/debug/seeed-hal-broker` | 9 checks passed |
| Manifest CLI | `target/debug/seeed-hal-broker --manifest` | v0.1.0, wire 1.0 inclusive range, virtual adapter, SHA-256 present |
