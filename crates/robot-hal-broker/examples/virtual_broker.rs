...
use robot_hal_broker::{Broker, StartupToken};
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    let token_path = std::env::var_os("ROBOT_HAL_TEST_TOKEN_FILE")
        .ok_or("ROBOT_HAL_TEST_TOKEN_FILE is required")?;
    ...
    let runtime = HalRuntime::builder()
        ...
        .usb_adapter(VirtualUsbAdapter::loopback("usb:virtual:python"))
        .gpio_adapter(VirtualGpioAdapter::line_bank("gpio:virtual:python", 2))
        .build();
...
async fn serve_one(broker: Broker) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let directory = std::env::var_os("ROBOT_HAL_TEST_DIRECTORY")
        .ok_or("ROBOT_HAL_TEST_DIRECTORY is required")?;
...
#[cfg(windows)]
async fn serve_one(broker: Broker) -> Result<(), Box<dyn std::error::Error>> {
    use robot_hal_broker::listener::WindowsBroker;
