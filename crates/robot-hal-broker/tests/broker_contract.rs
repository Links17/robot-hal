...
use prost::Message;
use robot_hal_broker::{Broker, BrokerConfig, StartupToken};
use robot_hal_can::{
...
use robot_hal_gpio::GpioEdge;
use robot_hal_protocol::v1::{self, envelope};
use robot_hal_runtime::{HalRuntime, RuntimeEventKind};
use robot_hal_serial::{ControlLines, SerialAdapter, SerialConfig, SerialSession};
...
async fn handshake_version_capability_and_byte_limits_fail_closed() {
    ...
    oversized_frame.max_frame_bytes = (robot_hal_protocol::MAX_FRAME_BYTES + 1) as u32;
    assert_eq!(
...
async fn handshake_frame_length_prefix_over_hard_cap_fails_before_decode() {
    ...
    let oversized_len = u32::try_from(robot_hal_protocol::MAX_FRAME_BYTES + 1).unwrap();
    client_io
...
async fn disconnect_restores_configured_can_and_releases_can_and_serial() {
    ...
    runtime
        .open_can(
            ...
            robot_hal_core::LeaseMode::Control,
            ...
            robot_hal_can::CanFilterSet::new(vec![]).unwrap(),
        )
        .await
        .unwrap()
        .close()
...
async fn unix_listener_uses_a_private_directory_and_socket() {
    ...
    use robot_hal_broker::listener::UnixBroker;
