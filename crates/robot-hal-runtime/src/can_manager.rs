...
impl CanManager {
    ...
    async fn discovery_records(&self, operation: &'static str) -> HalResult<Vec<DiscoveryRecord>> {
        ...
        for (adapter_index, adapter) in self.adapters.iter().enumerate() {
            ...
            let (reply_tx, reply_rx) = oneshot::channel();
            let name = format!("robot-hal-can-enumerate-{adapter_index}");
...
fn required_capability(config: &CanOpenConfig) -> robot_hal_core::CapabilityId {
    match config {
...
fn session_closed(operation: &'static str, resource_id: &ResourceId) -> robot_hal_core::HalError {
    runtime_error(
        "runtime.session.closed",
        ErrorCategory::Conflict,
        operation,
...
fn session_not_found(operation: &'static str) -> robot_hal_core::HalError {
    runtime_error(
...
mod tests {
    ...
    use robot_hal_core::{ErrorCategory, HalError, LeaseMode, OwnerId};
    ...
    async fn non_first_provisional_cleanup_finishes_when_actor_terminates_after_admission() {
        ...
        let session_id = robot_hal_core::SessionId::parse("cleanup-termination").unwrap();
