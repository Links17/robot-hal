...
use robot_hal_core::{LeaseMode, LeaseToken, OwnerId};
use robot_hal_runtime::HalRuntime;
use robot_hal_testkit::VirtualCameraAdapter;
...
async fn camera_runtime_is_exclusive_fences_stale_leases_and_publishes_a_ring_frame() {
    ...
    let mut reader = robot_hal_adapter_shared_memory::ReadOnlyMapping::open(&mapping).unwrap();
    assert_eq!(
