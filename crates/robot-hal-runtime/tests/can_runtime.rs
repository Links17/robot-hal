...
use robot_hal_testkit::{VirtualCanAdapter, VirtualSerialAdapter};
...
async fn wait_for_can_session_closed(handle: &robot_hal_runtime::CanHandle) {
...
async fn enqueue_blocked_filter_command(
    runtime: &HalRuntime,
    handle: &robot_hal_runtime::CanHandle,
...
async fn wait_for_close_admission(
    runtime: &HalRuntime,
    session: robot_hal_core::SessionId,
    token: &robot_hal_core::LeaseToken,
) {
    tokio::time::timeout(Duration::from_secs(1), async {
        loop {
            match runtime
...
async fn backend_methods_never_run_on_tokio_workers() {
    ...
    assert!(
        state
            .thread_names
            .iter()
            .all(|name| name.starts_with("robot-hal-can-"))
    );
}
...
fn descriptor_with_capability(
    resource_id: &str,
    capability: robot_hal_core::CapabilityId,
) -> ResourceDescriptor {
