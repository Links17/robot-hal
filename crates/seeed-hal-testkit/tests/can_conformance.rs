...
use seeed_hal_testkit::{VirtualCanAdapter, run_can_adapter_conformance};
...
async fn injected_timestamp_and_status_are_observable() {
    ...
    adapter.set_bus_status(seeed_hal_can::CanBusStatus::new(
        CanBusState::Warning,
        Some(3),
        Some(4),
    ));
...
async fn configure_is_exclusive_and_restored_on_close() {
    ...
    let attached = adapter
        .open(
            &descriptor.selector(),
            &CanOpenConfig::Attach(
                seeed_hal_can::CanLinkExpectation::new(
...
async fn descriptor_is_strong_can_and_advertises_exact_capabilities() {
    ...
    assert_eq!(
        ...
        seeed_hal_can::IdentityQuality::Strong
    );
    for capability in [
