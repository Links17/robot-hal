...
use robot_hal_gpio::{GpioDrive, GpioEdge, GpioEdgeEvent, GpioLineConfig};
use robot_hal_protocol::v1::{self, envelope};

#[test]
fn wire_minor_three_is_the_latest_additive_protocol_version() {
    assert_eq!(robot_hal_protocol::PROTOCOL_MINOR_MAXIMUM, 3);
    assert_eq!(robot_hal_protocol::PROTOCOL_MINOR, 3);
}
...
fn usb_transfer_decoder_rejects_payload_over_public_bound() {
    let error = robot_hal_protocol::usb_transfer_from_proto(v1::UsbTransferRequest {
...
fn gpio_config_decoder_rejects_unspecified_direction() {
    let error =
        robot_hal_protocol::gpio_config_from_proto(v1::GpioLineConfig::default()).unwrap_err();
    assert_eq!(error.name().as_str(), "runtime.protocol.invalid_message");
}

#[test]
fn usb_and_gpio_minor_two_responses_round_trip_sessions_and_domain_values() {
    let session = robot_hal_core::SessionId::parse("session-usb-gpio").unwrap();
    ...
    assert_eq!(
        robot_hal_protocol::open_usb_response_from_proto(
            robot_hal_protocol::open_usb_response_to_proto(&session, &lease)
        )
        .unwrap(),
        (session.clone(), lease.clone())
    );
    assert_eq!(
        robot_hal_protocol::open_gpio_response_from_proto(
            robot_hal_protocol::open_gpio_response_to_proto(&session, &lease)
        )
        .unwrap(),
        (session, lease)
    );
    ...
    assert_eq!(
        robot_hal_protocol::gpio_config_from_proto(v1::GpioLineConfig::from(&config)).unwrap(),
        config
    );
    let event = GpioEdgeEvent::new(GpioEdge::Falling, 42, 3);
    assert_eq!(
        robot_hal_protocol::gpio_edge_event_from_proto(v1::GpioEdgeEvent::from(&event)).unwrap(),
        event
    );
}
...
fn usb_and_gpio_minor_two_requests_reject_transport_and_public_bounds() {
    ...
    assert_invalid_message(
        robot_hal_protocol::open_usb_request_from_proto(v1::OpenUsbRequest {
    ...
    let error = robot_hal_protocol::gpio_edge_request_from_proto(v1::GpioNextEdgeRequest {
        rising: true,
        capacity: 1025,
        ..Default::default()
    })
...
fn usb_and_gpio_operations_validate_session_leases_timeouts_and_payloads() {
    for lease in [None, Some(v1::LeaseToken::default())] {
        let error = robot_hal_protocol::usb_transfer_request_from_proto(v1::UsbTransferRequest {
    ...
    for request in [
        ...
        assert_eq!(
            robot_hal_protocol::usb_transfer_request_from_proto(request)
                .unwrap_err()
                .name()
                .as_str(),
    ...
    assert_eq!(
        robot_hal_protocol::gpio_read_request_from_proto(v1::GpioReadRequest {
    ...
    assert_eq!(
        robot_hal_protocol::gpio_write_request_from_proto(v1::GpioWriteRequest {
    ...
    assert_eq!(
        robot_hal_protocol::gpio_next_edge_request_from_proto(v1::GpioNextEdgeRequest {
            session_id: "session-gpio".to_owned(),
            lease: Some(valid_lease()),
            rising: true,
            capacity: 1,
...
fn usb_gpio_response_and_close_conversions_round_trip_without_defaulting() {
    let session = robot_hal_core::SessionId::parse("session-usb-gpio-ops").unwrap();
    let lease = LeaseToken::new(
    ...
    assert_eq!(
        robot_hal_protocol::usb_close_request_from_proto(v1::CloseUsbRequest {
    ...
    assert_eq!(
        robot_hal_protocol::gpio_close_request_from_proto(v1::CloseGpioRequest {
            session_id: session.as_str().to_owned(),
            lease: Some((&lease).into()),
        })
        .unwrap(),
    ...
    assert_eq!(
        robot_hal_protocol::usb_transfer_response_from_proto(
            robot_hal_protocol::usb_transfer_response_to_proto(bytes::Bytes::from_static(b"usb"))
        )
        .unwrap(),
        bytes::Bytes::from_static(b"usb")
    );
    assert_eq!(
        robot_hal_protocol::gpio_read_response_from_proto(
            robot_hal_protocol::gpio_read_response_to_proto(&[true, false])
        )
        .unwrap(),
        vec![true, false]
    );
    ...
    assert_eq!(
        robot_hal_protocol::gpio_next_edge_response_from_proto(
            robot_hal_protocol::gpio_next_edge_response_to_proto(Some(event))
        )
        .unwrap(),
        Some(event)
    );
    assert_eq!(
        robot_hal_protocol::gpio_next_edge_response_from_proto(
            robot_hal_protocol::gpio_next_edge_response_to_proto(None)
        )
        .unwrap(),
        None
    );
...
fn legacy_exact_minor_defaults_to_a_single_value_range() {
    ...
    assert_eq!(
        robot_hal_protocol::handshake_minor_range(&request).unwrap(),
        (0, 0)
    );
}
...
fn highest_shared_minor_is_selected_from_inclusive_ranges() {
    assert_eq!(
        robot_hal_protocol::negotiate_protocol_minor(1, 1, 3, 1, 2, 4).unwrap(),
        3,
    );
}
...
fn no_overlap_and_major_mismatch_use_the_stable_compatibility_error() {
    for result in [
        robot_hal_protocol::negotiate_protocol_minor(1, 0, 1, 1, 2, 3),
        robot_hal_protocol::negotiate_protocol_minor(1, 0, 3, 2, 0, 3),
    ] {
        assert_eq!(
...
fn unknown_additive_handshake_fields_are_ignored_safely() {
    ...
    assert_eq!(
        robot_hal_protocol::handshake_minor_range(&decoded).unwrap(),
        (0, 0)
    );
}
...
fn invalid_required_enum_is_a_structured_protocol_error() {
    ...
    let error = robot_hal_core::ResourceSelector::try_from(selector).unwrap_err();
    assert_eq!(error.name().as_str(), "runtime.protocol.invalid_message");
}
...
fn rich_error_round_trips_all_structured_details() {
    ...
    let decoded = robot_hal_protocol::error_from_proto(v1::Error::from(&error)).unwrap();
...
fn legacy_error_round_trip_has_empty_structured_details() {
    let decoded = robot_hal_protocol::error_from_proto(valid_error()).unwrap();
...
fn malformed_error_details_are_invalid_messages() {
    let invalid_resource = robot_hal_protocol::error_from_proto(v1::Error {
    ...
    for (field, value) in [("platform_code", "é"), ("vendor_code", "é")] {
        ...
        let error = robot_hal_protocol::error_from_proto(result).unwrap_err();
        assert_eq!(error.name().as_str(), "runtime.protocol.invalid_message");
        assert!(error.debug_message().contains(field));
    }

    let error = robot_hal_protocol::error_from_proto(v1::Error {
        context: [("QueueDepth".to_owned(), "64".to_owned())].into(),
        ..valid_error()
    })
    .unwrap_err();
...
fn malformed_error_details_reject_every_size_and_count_bound() {
    ...
    for value in cases {
        let error = robot_hal_protocol::error_from_proto(value).unwrap_err();
        assert_eq!(error.name().as_str(), "runtime.protocol.invalid_message");
    }
}
...
fn wire_minor_three_range_and_additive_enum_values_are_locked() {
    assert_eq!(robot_hal_protocol::PROTOCOL_MAJOR, 1);
    assert_eq!(robot_hal_protocol::PROTOCOL_MINOR_MINIMUM, 0);
    assert_eq!(robot_hal_protocol::PROTOCOL_MINOR_MAXIMUM, 3);
    assert_eq!(robot_hal_protocol::PROTOCOL_MINOR, 3);
...
fn wire_one_keeps_legacy_exact_minor_zero_negotiation() {
    ...
    assert_eq!(
        robot_hal_protocol::handshake_minor_range(&legacy).unwrap(),
        (0, 0)
    );
    assert_eq!(
        robot_hal_protocol::negotiate_protocol_minor(
            legacy.protocol_major,
            0,
            0,
            robot_hal_protocol::PROTOCOL_MAJOR,
            robot_hal_protocol::PROTOCOL_MINOR_MINIMUM,
            robot_hal_protocol::PROTOCOL_MINOR_MAXIMUM,
        )
        .unwrap(),
        0,
    );
    assert_eq!(
        robot_hal_protocol::negotiate_protocol_minor(1, 0, 1, 1, 0, 1).unwrap(),
        1,
    );
}
...
fn camera_resources_round_trip_after_the_protocol_defines_camera_transport() {
    ...
    let descriptor = robot_hal_core::ResourceDescriptor::new(
        selector.id().clone(),
        robot_hal_core::Endpoint::new("virtual://camera/protocol-boundary").unwrap(),
        IdentityQuality::Strong,
        TransportKind::Camera,
        robot_hal_core::ResourceProperties::default(),
        robot_hal_core::CapabilitySet::new(vec![
            robot_hal_core::CapabilityId::parse("camera.capture/v1").unwrap(),
        ]),
    );
    assert_eq!(
        robot_hal_core::ResourceDescriptor::try_from(
...
fn camera_minor_three_defines_additive_transport_and_payload_tags_without_frame_bytes() {
    ...
    let secure_descriptor = robot_hal_protocol::camera_mapping_descriptor_from_proto(descriptor)
...
fn camera_next_frame_lease_decoder_rejects_zero_sequence_and_preserves_validated_wire_fields() {
    let zero_sequence = robot_hal_protocol::camera_next_frame_lease_response_from_proto(
    ...
    let lease = robot_hal_protocol::camera_next_frame_lease_response_from_proto(
        v1::CameraNextFrameLeaseResponse {
            lease: Some(v1::FrameLease {
                slot_index: 2,
                sequence: 7,
...
fn serial_operation_decoders_reject_can_and_maintenance_values() {
    ...
    assert_invalid_message(
        robot_hal_protocol::serial_selector_from_proto(can_selector).unwrap_err(),
    ...
    assert_invalid_message(
        robot_hal_protocol::enumerate_serial_response_from_proto(v1::EnumerateSerialResponse {
    ...
    assert_invalid_message(
        robot_hal_core::ResourceDescriptor::try_from(empty_can_descriptor).unwrap_err(),
    ...
    assert_invalid_message(
        robot_hal_protocol::open_serial_response_from_proto(maintenance).unwrap_err(),
...
fn exact_can_wire_bounds_accept_limits_and_reject_every_fd_gap() {
    ...
    assert!(
        CanFrame::try_from(v1::CanFrame {
            ...
            error_classes: vec![
                ...
                robot_hal_can::MAX_CAN_ERROR_CLASSES
            ],
            ..Default::default()
        })
        .is_ok()
    ...
    assert_invalid_message(
        CanFrame::try_from(v1::CanFrame {
            ...
            error_classes: vec![
                v1::CanErrorClass::Other as i32;
                robot_hal_can::MAX_CAN_ERROR_CLASSES + 1
            ],
            ..Default::default()
        })
        .unwrap_err(),
    ...
    assert!(robot_hal_protocol::send_can_frames_from_proto(vec![classic_frame(); 64]).is_ok());
    assert!(robot_hal_protocol::can_receive_parameters(64, u64::MAX).is_ok());
}
...
fn filter_batch_receive_and_required_nested_bounds_fail_closed() {
    ...
    assert_invalid_message(
        robot_hal_protocol::send_can_frames_from_proto(Vec::new()).unwrap_err(),
        "can_send.frames",
    );
    assert_invalid_message(
        robot_hal_protocol::send_can_frames_from_proto(vec![classic_frame(); 65]).unwrap_err(),
        "can_send.frames",
    );
    for max_frames in [0, 65] {
        assert_invalid_message(
            robot_hal_protocol::can_receive_parameters(max_frames, 0).unwrap_err(),
            "can_receive.max_frames",
        );
    }
    assert_eq!(
        robot_hal_protocol::can_receive_parameters(1, 0).unwrap().1,
        std::time::Duration::ZERO,
    );
    assert_invalid_message(
        robot_hal_protocol::received_can_frames_from_proto(
            vec![
                v1::ReceivedCanFrame {
                    frame: Some(classic_frame()),
                    timestamp: None,
    ...
    assert_invalid_message(
        robot_hal_protocol::get_can_bus_status_response_from_proto(
...
fn open_request_rejects_missing_values_and_non_can_transport() {
    ...
    for request in [
        ...
        assert_invalid_message(
            robot_hal_protocol::open_can_request_from_proto(request).unwrap_err(),
            "open_can",
        );
    }

    assert_invalid_message(
        robot_hal_protocol::enumerate_can_response_from_proto(v1::EnumerateCanResponse {
    ...
    assert_invalid_message(
        robot_hal_protocol::open_can_response_from_proto(
            v1::OpenCanResponse {
                session_id: "session".to_owned(),
                lease: Some(valid_lease()),
            },
...
fn send_response_validates_success_and_partial_error_against_input_length() {
    let success = robot_hal_protocol::can_send_response_from_proto(
        v1::CanSendResponse {
            committed_count: 2,
            error: None,
        },
    ...
    for response in [
        ...
        assert_invalid_message(
            robot_hal_protocol::can_send_response_from_proto(response, 2).unwrap_err(),
            "committed_count",
        );
    }
    ...
    let encoded = robot_hal_protocol::can_send_response_to_proto(Err(&partial), 2).unwrap();
    assert_eq!(encoded.committed_count, 1);
    let decoded = robot_hal_protocol::can_send_response_from_proto(encoded, 2)
        .unwrap()
        .unwrap_err();
    assert_eq!(decoded.committed(), 1);
    assert_eq!(decoded.error().name().as_str(), "runtime.transport.failed");

    let invalid_nested = robot_hal_protocol::can_send_response_from_proto(
        v1::CanSendResponse {
            committed_count: 0,
