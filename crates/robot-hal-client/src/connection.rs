    ...
    v1::CameraRequest {
        format: Some(v1::CameraFormat {
            pixel_format: match request.format().pixel_format() {
                robot_hal_camera::CameraPixelFormat::Nv12 => v1::CameraPixelFormat::Nv12,
                robot_hal_camera::CameraPixelFormat::Yuyv => v1::CameraPixelFormat::Yuyv,
                robot_hal_camera::CameraPixelFormat::Mjpeg => v1::CameraPixelFormat::Mjpeg,
            } as i32,
...
impl HalClient {
    ...
    pub async fn open_serial(
        ...
        if selector.transport() != robot_hal_core::TransportKind::Serial {
            return Err(client_error(
                "runtime.argument.invalid",
                ErrorCategory::InvalidArgument,
                "serial.open",
    ...
    pub async fn open_usb(
        ...
        if selector.transport() != robot_hal_core::TransportKind::Usb {
            return Err(client_error(
                "runtime.argument.invalid",
                ErrorCategory::InvalidArgument,
                "usb.open",
    ...
    pub async fn open_gpio(
        ...
        if selector.transport() != robot_hal_core::TransportKind::Gpio
            || lines.is_empty()
    ...
    pub async fn open_camera(
        ...
        if selector.transport() != robot_hal_core::TransportKind::Camera {
            return Err(client_error(
                "runtime.argument.invalid",
                ErrorCategory::InvalidArgument,
                "camera.open",
    ...
    pub async fn open_can(
        ...
        if selector.transport() != robot_hal_core::TransportKind::Can {
            return Err(client_error(
                "runtime.argument.invalid",
                ErrorCategory::InvalidArgument,
                "can.open",
    ...
    fn require_can_capability(
        &self,
        operation: &'static str,
        resource_id: Option<&robot_hal_core::ResourceId>,
    ...
    match (expected, payload) {
        ...
        (ExpectedResponse::EnumerateUsb, envelope::Payload::EnumerateUsbResponse(response)) => {
            response.resources.iter().try_for_each(|resource| {
                ...
                descriptor.and_then(|descriptor| {
                    (descriptor.transport() == robot_hal_core::TransportKind::Usb)
                        .then_some(())
                        .ok_or_else(|| {
                            robot_hal_protocol::invalid_message(
        ...
        (ExpectedResponse::EnumerateGpio, envelope::Payload::EnumerateGpioResponse(response)) => {
            response.resources.iter().try_for_each(|resource| {
                let descriptor: HalResult<ResourceDescriptor> = resource.clone().try_into();
                descriptor.and_then(|descriptor| {
                    (descriptor.transport() == robot_hal_core::TransportKind::Gpio)
                        .then_some(())
                        .ok_or_else(|| {
                            robot_hal_protocol::invalid_message(
        ...
        (
            ...
            (line_count > 0).then_some(()).ok_or_else(|| {
                attach_resource(
                    robot_hal_protocol::invalid_message("GPIO open line count is invalid"),
                    &resource_id,
                )
            })?;
        ...
        (
            ...
            let descriptor: HalResult<ResourceDescriptor> = resource.clone().try_into();
            descriptor.and_then(|descriptor| {
                (descriptor.transport() == robot_hal_core::TransportKind::Camera)
                    .then_some(())
                    .ok_or_else(|| {
                        robot_hal_protocol::invalid_message(
        ...
        ) => robot_hal_protocol::camera_open_response_from_proto(response.clone())
            .map(|_| ())
            .map_err(|error| attach_resource(error, &resource_id)),
...
fn validate_received_profile(
    frames: &[robot_hal_can::ReceivedCanFrame],
    profile: &CanSessionProfile,
) -> HalResult<()> {
    for received in frames {
        let frame_allowed = match received.frame() {
            robot_hal_can::CanFrame::ClassicData { .. }
            | robot_hal_can::CanFrame::ClassicRemote { .. } => profile.classic_frames,
            robot_hal_can::CanFrame::FdData { .. } => {
                profile.fd_frames && profile.mode == CanMode::Fd
            }
            robot_hal_can::CanFrame::Error { .. } => profile.error_frames,
        };
        if !frame_allowed {
            return Err(invalid_profile_message(
                profile,
...
fn invalid_profile_message(profile: &CanSessionProfile, message: &'static str) -> HalError {
    attach_profile(
        robot_hal_protocol::invalid_message(format!(
...
fn attach_resource(error: HalError, resource_id: &robot_hal_core::ResourceId) -> HalError {
...
fn select_can_descriptor<'a>(
    ...
    let mut matches = resources.iter().filter(|resource| {
        resource.id() == selector.id()
            && resource.transport() == robot_hal_core::TransportKind::Can
            && resource
...
mod tests {
    ...
    use robot_hal_protocol::PROTOCOL_MINOR_MAXIMUM;
    use robot_hal_protocol::v1::{self, envelope};
    ...
    fn camera_response_validation_rejects_invalid_associated_payloads() {
        let resource_id = robot_hal_core::ResourceId::parse("camera:virtual:validation").unwrap();
    ...
    async fn writer_queue_overflow_uses_a_positively_blocked_writer() {
        ...
        let server = tokio::spawn(async move {
            let mut wire = tokio_util::codec::Framed::new(
                server_io,
                frame_codec(robot_hal_protocol::MAX_FRAME_BYTES),
            );
