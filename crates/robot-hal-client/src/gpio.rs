...
use robot_hal_gpio::{GpioEdgeEvent, GpioEdgeRequest, MAX_GPIO_EVENTS};
...
impl RemoteGpioHandle {
    ...
    pub async fn next_edge(
        ...
        let payload = envelope::Payload::GpioNextEdgeRequest(v1::GpioNextEdgeRequest {
            session_id: self.session_id.as_str().to_owned(),
            lease: Some((&self.lease).into()),
            rising: request.edges().contains(robot_hal_gpio::GpioEdge::Rising),
            falling: request.edges().contains(robot_hal_gpio::GpioEdge::Falling),
