...
use robot_hal_testkit::{VirtualCanAdapter, run_can_adapter_conformance};
...
async fn frames_preserve_order_and_metadata() {
    ...
    let mut channel = adapter
        .open(
            ...
            &CanOpenConfig::Attach(
                robot_hal_can::CanLinkExpectation::new(None, None, None, None, None).unwrap(),
            ),
        )
        .await
        .unwrap();
    let standard =
        CanFrame::classic_data(CanId::standard(1).unwrap(), Bytes::from_static(&[1])).unwrap();
    let extended = CanFrame::fd_data(
        CanId::extended(2).unwrap(),
        Bytes::from_static(&[2; 12]),
        true,
        true,
    )
    .unwrap();
    ...
    let error = CanFrame::error(
        vec![robot_hal_can::CanErrorClass::BusError],
        Bytes::from_static(&[9]),
    )
    .unwrap();
    for frame in [&standard, &extended, &remote, &error] {
    ...
    assert_eq!(
        adapter.transmitted_frames(),
        vec![standard, extended, remote, error]
    );
}
...
async fn injected_timestamp_and_status_are_observable() {
    ...
    adapter
        .inject_received(frame.clone(), Some(timestamp.clone()))
        .unwrap();
    ...
    adapter.set_bus_status(robot_hal_can::CanBusStatus::new(
        CanBusState::Warning,
        Some(3),
        Some(4),
    ));
...
async fn configure_is_exclusive_and_restored_on_close() {
    ...
    let conflict = match adapter
        ...
        .await
    {
        Ok(_) => panic!("concurrent Configure must fail"),
        Err(error) => error,
    };
    ...
    let attached = adapter
        .open(
            &descriptor.selector(),
            &CanOpenConfig::Attach(
                robot_hal_can::CanLinkExpectation::new(
...
async fn receive_timeout_and_close_are_finite_and_structured() {
    ...
    assert_eq!(
        channel
            .send(&CanFrame::classic_remote(CanId::standard(1).unwrap(), 0).unwrap())
            .unwrap_err()
            .name()
            .as_str(),
...
async fn receive_overflow_reports_lag_before_retained_frames() {
        ...
        adapter
            .inject_received(
                CanFrame::classic_data(
                    CanId::standard(value & 0x7ff).unwrap(),
                    Bytes::from_static(&[1]),
                )
                .unwrap(),
                None,
    ...
    assert_eq!(
        lagged.context().iter().collect::<Vec<_>>(),
        vec![("dropped_count", "1")]
    );
...
async fn descriptor_is_strong_can_and_advertises_exact_capabilities() {
    ...
    let descriptor = adapter.enumerate().await.unwrap().remove(0);
    assert_eq!(descriptor.transport(), robot_hal_can::TransportKind::Can);
    assert_eq!(
        descriptor.minimum_identity_quality(),
        robot_hal_can::IdentityQuality::Strong
    );
    for capability in [
        robot_hal_can::can_classic_capability(),
        robot_hal_can::can_fd_capability(),
        robot_hal_can::can_configure_capability(),
        robot_hal_can::can_error_frames_capability(),
        robot_hal_can::can_rx_timestamp_capability(),
    ] {
