...
impl VirtualCameraAdapter {
    ...
    fn new(resource_id: impl Into<String>, controls_supported: bool) -> Self {
        ...
        let descriptor = ResourceDescriptor::new(
            id.clone(),
            robot_hal_core::Endpoint::new(format!("virtual://camera/{}", id.as_str()))
...
pub async fn run_camera_adapter_conformance<A: CameraAdapter>(adapter: &A) -> HalResult<()> {
    ...
    let request = CameraRequest::new(
        ...
        robot_hal_camera::DEFAULT_CAMERA_SLOT_COUNT,
    )?;
    ...
    assert!(first.payload().len() <= robot_hal_camera::MAX_CAMERA_FRAME_BYTES);
