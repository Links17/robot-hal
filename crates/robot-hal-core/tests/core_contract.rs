...
fn error_decisions_do_not_require_message_parsing() {
    ...
    assert_eq!(error.category(), robot_hal_core::ErrorCategory::Conflict);
...
fn malformed_serialized_values_are_rejected() {
    assert!(serde_json::from_str::<robot_hal_core::ResourceId>("\"\"").is_err());
    assert!(serde_json::from_str::<robot_hal_core::Endpoint>("\"\"").is_err());
    assert!(serde_json::from_str::<robot_hal_core::CapabilityId>("\"serial.bytes\"").is_err());
    assert!(serde_json::from_str::<robot_hal_core::LeaseId>("\"\"").is_err());
    assert!(serde_json::from_str::<robot_hal_core::ErrorName>("\"\"").is_err());
}
...
fn serialized_hal_error_shape_deserializes_and_reserializes_compatibly() {
    let original = robot_hal_core::HalError::new(
        "runtime.lease.stale_generation",
        robot_hal_core::ErrorCategory::Conflict,
        "serial.write",
        false,
    ...
    let restored: robot_hal_core::HalError = serde_json::from_value(json.clone()).unwrap();
